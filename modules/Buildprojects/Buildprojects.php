...
class Buildprojects extends Vtiger_CRMEntity {
	...
	var $default_sort_order='ASC';
        ...
        function get_tenants($id, $cur_tab_id, $rel_tab_id, $actions=false) {
                global $log, $singlepane_view,$currentModule,$current_user;
                $log->debug("Entering get_tenants(".$id.") method ...");
                $this_module = $currentModule;
        ...
        vtlib_setup_modulevars($related_module, $other);
                ...
                $query = "SELECT vtiger_crmentity.*,
                          ...
                          FROM   vtiger_tenants 
                          INNER JOIN vtiger_crmentity
                            ON   vtiger_crmentity.crmid = vtiger_tenants.tenantsid
                          LEFT  JOIN vtiger_tenantscf
                            ON   vtiger_tenantscf.tenantsid = vtiger_tenants.tenantsid
                          WHERE  vtiger_crmentity.deleted = 0
                            AND  vtiger_tenants.tenantbuildprojects = ".$id;
                ...
                $log->debug("Exiting get_tenants method ...");
                return $return_value;
        }
